...
class Booking extends CI_Controller
{
    ...
    public function hapusbooking()
    {
        ...
        // Menghapus data buku dari keranjang
        $this->ModelBooking->deleteData(['id_buku' => $id_buku], 'temp');

        // Mengecek apakah keranjang sudah kosong
    ...
    {
        // Mengupdate stok dan jumlah dibooking di tabel buku saat proses booking diselesaikan
        $this->db->query(
            "UPDATE buku, temp 
            SET buku.dibooking = buku.dibooking + 1, buku.stok = buku.stok - 1 
        ...
        // Menyimpan ke tabel booking dan detail booking, serta mengosongkan tabel temporari
        $this->ModelBooking->insertData('booking', $isiBooking);
        $this->ModelBooking->simpanDetail($where);
        $this->ModelBooking->kosongkanData('temp');

        // Mengarahkan kembali ke halaman informasi booking
    ...
    public function info()
    {
        ...
        $data['useraktif'] = $this->ModelUser->cekData(['id' => $where])->result();
        $data['items'] = $this->db->query(
        "SELECT * 
        FROM booking bo, booking_detail d, buku bu 
        WHERE d.id_booking = bo.id_booking 
        AND d.id_buku = bu.id 
        AND bo.id_user = '$where'"
        )->result_array();

    // Memuat tampilan halaman dengan data yang sudah disiapkan
        $this->load->view('templates/templates-user/header', $data);
